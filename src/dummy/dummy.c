...
#include "drivers/dummy/dummy_driver.h"

PG_MODULE_MAGIC;

PGDLLEXPORT Datum one_to_one_dummy(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(one_to_one_dummy);
...
process(
    ...
    if (only_cost) {
        time_msg("Invalid", start_t, clock());
    } else {
        time_msg("processing pgr_dummy", start_t, clock());
    }
...
PGDLLEXPORT Datum
one_to_one_dummy(PG_FUNCTION_ARGS) {
